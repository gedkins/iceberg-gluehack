...
public class GlueCatalog extends BaseMetastoreCatalog implements Closeable, SupportsNamespaces, Configurable {
    ...
    public void initialize(String name, Map<String, String> properties) {
        initialize(name, properties.get(CatalogProperties.WAREHOUSE_LOCATION), new AwsProperties(properties),
                AwsClientFactories.from(properties).glue(), LockManagers.from(properties),
    ...
    @VisibleForTesting
    void initialize(String name, String path, AwsProperties properties, GlueClient client, LockManager lock,
            FileIO io) {
    ...
    protected String defaultWarehouseLocation(TableIdentifier tableIdentifier) {
        ...
        GetDatabaseResponse response = glue.getDatabase(
                GetDatabaseRequest.builder().name(IcebergToGlueConverter.getDatabaseName(tableIdentifier)).build());
        ...
        return String.format("%s/%s.db/%s", warehousePath, IcebergToGlueConverter.getDatabaseName(tableIdentifier),
    ...
    public List<TableIdentifier> listTables(Namespace namespace) {
        ...
        do {
            GetTablesResponse response = glue.getTables(GetTablesRequest.builder()
                    ...
                    .databaseName(IcebergToGlueConverter.toDatabaseName(namespace)).nextToken(nextToken).build());
            nextToken = response.nextToken();
            if (response.hasTableList()) {
                results.addAll(response.tableList().stream().map(GlueToIcebergConverter::toTableId)
    ...
    public boolean dropTable(TableIdentifier identifier, boolean purge) {
        try {
            ...
            glue.deleteTable(DeleteTableRequest.builder().catalogId(awsProperties.glueCatalogId())
                    .databaseName(IcebergToGlueConverter.getDatabaseName(identifier)).name(identifier.name()).build());
    ...
    public void renameTable(TableIdentifier from, TableIdentifier to) {
        ...
        if (!namespaceExists(to.namespace())) {
            throw new NoSuchNamespaceException("Cannot rename %s to %s because namespace %s does not exist", from, to,
                    to.namespace());
        }
        // keep metadata
        ...
        try {
            GetTableResponse response = glue.getTable(GetTableRequest.builder().catalogId(awsProperties.glueCatalogId())
                    .databaseName(fromTableDbName).name(fromTableName).build());
        ...
        TableInput.Builder tableInputBuilder = TableInput.builder().owner(fromTable.owner())
                .tableType(fromTable.tableType()).parameters(fromTable.parameters());

        glue.createTable(CreateTableRequest.builder().catalogId(awsProperties.glueCatalogId())
                .databaseName(toTableDbName).tableInput(tableInputBuilder.name(toTableName).build()).build());
        ...
        try {
            ...
            glue.deleteTable(DeleteTableRequest.builder().catalogId(awsProperties.glueCatalogId())
                    .databaseName(toTableDbName).name(toTableName).build());
            throw e;
        }
    ...
    public void createNamespace(Namespace namespace, Map<String, String> metadata) {
        try {
            glue.createDatabase(CreateDatabaseRequest.builder().catalogId(awsProperties.glueCatalogId())
                    .databaseInput(IcebergToGlueConverter.toDatabaseInput(namespace, metadata)).build());
    ...
    public List<Namespace> listNamespaces(Namespace namespace) throws NoSuchNamespaceException {
        ...
        do {
            GetDatabasesResponse response = glue.getDatabases(GetDatabasesRequest.builder()
                    .catalogId(awsProperties.glueCatalogId()).nextToken(nextToken).build());
            nextToken = response.nextToken();
            if (response.hasDatabaseList()) {
                results.addAll(response.databaseList().stream().map(GlueToIcebergConverter::toNamespace)
                        .collect(Collectors.toList()));
            }
        } while (nextToken != null);
    ...
    public Map<String, String> loadNamespaceMetadata(Namespace namespace) throws NoSuchNamespaceException {
        ...
        try {
            GetDatabaseResponse response = glue.getDatabase(
                    GetDatabaseRequest.builder().catalogId(awsProperties.glueCatalogId()).name(databaseName).build());
            Map<String, String> result = response.database().parameters();
            LOG.debug("Loaded metadata for namespace {} found {}", namespace, result);
            return result;
        } catch (InvalidInputException e) {
            throw new NoSuchNamespaceException("invalid input for namespace %s, error message: %s", namespace,
                    e.getMessage());
        } catch (EntityNotFoundException e) {
    ...
    public boolean dropNamespace(Namespace namespace) throws NamespaceNotEmptyException {
        ...
        glue.deleteDatabase(DeleteDatabaseRequest.builder().catalogId(awsProperties.glueCatalogId())
                .name(IcebergToGlueConverter.toDatabaseName(namespace)).build());
    ...
    public boolean setProperties(Namespace namespace, Map<String, String> properties) throws NoSuchNamespaceException {
        ...
        glue.updateDatabase(UpdateDatabaseRequest.builder().catalogId(awsProperties.glueCatalogId())
                ...
                .databaseInput(IcebergToGlueConverter.toDatabaseInput(namespace, newProperties)).build());
