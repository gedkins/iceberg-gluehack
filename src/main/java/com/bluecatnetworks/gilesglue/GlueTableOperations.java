...
class GlueTableOperations extends BaseMetastoreTableOperations {
    ...
    protected void doCommit(TableMetadata base, TableMetadata metadata) {
        ...
        try {
            ...
            throw new CommitFailedException(e, "Cannot commit %s because unexpected exception contacting AWS",
                    tableName());
        } finally {
    ...
    private Map<String, String> prepareProperties(Table glueTable, String newMetadataLocation) {
        Map<String, String> properties = glueTable != null ? Maps.newHashMap(glueTable.parameters())
                : Maps.newHashMap();
    ...
    private void persistGlueTable(Table glueTable, Map<String, String> parameters) {
        StorageDescriptor dummyStorageDescriptor = StorageDescriptor.builder().serdeInfo(SerDeInfo.builder().build())
                .build();
        if (glueTable != null) {
            LOG.debug("Committing existing Glue table: {}", tableName());
            glue.updateTable(UpdateTableRequest.builder().catalogId(awsProperties.glueCatalogId())
                    .databaseName(databaseName).skipArchive(awsProperties.glueCatalogSkipArchive())
                    .tableInput(TableInput.builder().name(tableName).tableType(GLUE_EXTERNAL_TABLE_TYPE)
                            .parameters(parameters).storageDescriptor(dummyStorageDescriptor).build())
                    .build());
        } else {
            LOG.debug("Committing new Glue table: {}", tableName());
            glue.createTable(
                    CreateTableRequest.builder().catalogId(awsProperties.glueCatalogId()).databaseName(databaseName)
                            .tableInput(TableInput.builder().name(tableName).tableType(GLUE_EXTERNAL_TABLE_TYPE)
                                    .parameters(parameters).storageDescriptor(dummyStorageDescriptor).build())
                            .build());
        }
