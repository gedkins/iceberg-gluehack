...
class LockManagers {
    ...
    abstract static class BaseLockManager implements LockManager {
        ...
        public ScheduledExecutorService scheduler() {
            if (scheduler == null) {
                synchronized (BaseLockManager.class) {
                    if (scheduler == null) {
                        scheduler = MoreExecutors
                                .getExitingScheduledExecutorService((ScheduledThreadPoolExecutor) Executors
                                        .newScheduledThreadPool(heartbeatThreads(), new ThreadFactoryBuilder()
                                                .setDaemon(true).setNameFormat("iceberg-lock-manager-%d").build()));
                    }
                }
            }
        ...
        public void initialize(Map<String, String> properties) {
            this.acquireTimeoutMs = PropertyUtil.propertyAsLong(properties, CatalogProperties.LOCK_ACQUIRE_TIMEOUT_MS,
            ...
            this.heartbeatThreads = PropertyUtil.propertyAsInt(properties, CatalogProperties.LOCK_HEARTBEAT_THREADS,
    ...
    static class InMemoryLockManager extends BaseLockManager {
        ...
        void acquireOnce(String entityId, String ownerId) {
            ...
            boolean succeed;
            if (content == null) {
                InMemoryLockContent previous = LOCKS.putIfAbsent(entityId,
                        new InMemoryLockContent(ownerId, expiration));
        ...
        public boolean acquire(String entityId, String ownerId) {
            try {
                Tasks.foreach(entityId).retry(Integer.MAX_VALUE - 1).onlyRetryOn(IllegalStateException.class)
